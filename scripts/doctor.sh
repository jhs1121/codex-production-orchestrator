...
required=(
  "$SKILL_ROOT/codex-production-orchestrator/SKILL.md"
  "$SKILL_ROOT/codex-production-orchestrator/references/model-routing.md"
  "$SKILL_ROOT/codex-production-orchestrator/references/execution-rules.md"
  "$SKILL_ROOT/codex-production-orchestrator/references/resume-and-checkpoints.md"
  "$SKILL_ROOT/codex-ctf-orchestrator/SKILL.md"
  "$SKILL_ROOT/codex-ctf-orchestrator/references/scope-and-safety.md"
  "$SKILL_ROOT/codex-ctf-orchestrator/references/toolchain-build.md"
  "$SKILL_ROOT/codex-ctf-orchestrator/references/challenge-routing.md"
  "$SKILL_ROOT/codex-ctf-orchestrator/references/worker-packets.md"
  "$SKILL_ROOT/codex-ctf-orchestrator/references/checkpoints-and-recovery.md"
  "$SKILL_ROOT/codex-ctf-orchestrator/assets/CTF_TOOL_SCOPE.template.md"
  "$SKILL_ROOT/codex-ctf-orchestrator/assets/toolchain-state.template.md"
)
...
profile_dir = Path(sys.argv[2]) if sys.argv[2] else None
...
pinned = {
    ...
    "terra-explorer.toml": ("gpt-5.6-terra", "max", "read-only"),
    ...
    "ctf-luna-worker.toml": ("gpt-5.6-luna", "max", "workspace-write"),
    "ctf-terra-triage.toml": ("gpt-5.6-terra", "max", "read-only"),
...
unpinned = {
    ...
    "parent-specialist.toml": "read-only",
    "parent-verifier.toml": "read-only",
    "ctf-parent-specialist.toml": "read-only",
    "ctf-parent-verifier.toml": "read-only",
}
...
for name, expected in pinned.items():
    ...
    if actual != expected or not data.get("developer_instructions"):
        raise SystemExit(f"INVALID {name}: {actual}")

for name, sandbox in unpinned.items():
    data = tomllib.loads((agent_dir / name).read_text(encoding="utf-8"))
    if "model" in data or "model_reasoning_effort" in data:
        raise SystemExit(f"INVALID {name}: must remain unpinned")
    if data.get("sandbox_mode") != sandbox or not data.get("developer_instructions"):
        raise SystemExit(f"INVALID {name}")

if profile_dir:
    expected_profiles = {
        "cpo-daily.config.toml": ("gpt-5.6-sol", "xhigh"),
        "cpo-terra.config.toml": ("gpt-5.6-terra", "max"),
        "cpo-quality.config.toml": ("gpt-5.6-sol", "max"),
        "cpo-ultra.config.toml": ("gpt-5.6-sol", "ultra"),
    }
    for name, expected in expected_profiles.items():
        data = tomllib.loads((profile_dir / name).read_text(encoding="utf-8"))
        if (data.get("model"), data.get("model_reasoning_effort")) != expected:
            raise SystemExit(f"INVALID profile {name}")
