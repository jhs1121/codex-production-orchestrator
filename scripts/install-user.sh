...
for arg in "$@"; do
  case "$arg" in
    ...
    -h|--help) printf 'Usage: bash scripts/install-user.sh [--default-auto]\n'; exit 0 ;;
...
for path in "$HOME/.agents" "$SKILL_ROOT" "$HOME/.codex" "$AGENT_DST"; do check_broken_symlink "$path"; done
...
owned_agents=(luna-builder.toml luna-explorer.toml luna-verifier.toml terra-explorer.toml parent-builder.toml parent-specialist.toml parent-verifier.toml sol-builder.toml sol-specialist.toml sol-verifier.toml ctf-luna-worker.toml ctf-terra-triage.toml ctf-parent-specialist.toml ctf-parent-verifier.toml)
...
For explicitly authorized CTF competitions, organizer-provided artifacts/targets, localhost, or owned local Docker/VM labs, also use codex-ctf-orchestrator. The CTF Skill owns authorization scope and target boundaries; Production owns execution discipline. Propagate the exact AUTH_SCOPE to every child. Never evade a safety/scope block through rewording, encoding, fragmentation, or another agent; checkpoint and continue only safe in-scope work.
